@php
$footerContactTitle = \App\Models\SiteSetting::get('footer_contact_title', 'Contact Us Now!');
...
$footerInfo1Title = \App\Models\SiteSetting::get('footer_info1_title', 'Information');
$footerInfo1List = \App\Models\SiteSetting::get('footer_info1_list', "Purchase|\nPayment|\nShipping|\nReturn|");
$footerInfo2Title = \App\Models\SiteSetting::get('footer_info2_title', 'Information');
$footerInfo2List = \App\Models\SiteSetting::get('footer_info2_list', "Hatchback|\nSedan|\nSUV|\nCrossover|");
$footerBrandsTitle = \App\Models\SiteSetting::get('footer_brands_title', 'Top Brand');
                ...
                <div class="footer__about">
                    ...
                    <div class="footer__social">
                        @foreach($socialLinks as $social)
                            <a href="{{ $social['url'] }}" class="{{ strtolower($social['platform'] ?? 'link') }}" target="_blank" rel="noopener" title="{{ $social['platform'] ?? '' }}">
                                <i class="fa fa-{{ $social['icon'] ?? 'link' }}"></i>
                            </a>
                        @endforeach
                    </div>
                </div>
                ...
                <div class="footer__brand">
                    ...
                    <ul>
                        @foreach($footerBrandItems as $item)
                            <li><a href="{{ route('cars', ['brand' => $item]) }}"><i class="fa fa-angle-right"></i> {{ $item }}</a></li>
                        @endforeach
                    </ul>
                </div>
            </div>
