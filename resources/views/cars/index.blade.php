...
@section('content')
    <style>
        ...
        .animate-fade-out {
            animation: fadeOutScale 0.5s ease-out forwards !important;
            pointer-events: none;
        }
                    ...
                    <h1 class="page-title">
                        @if(Auth::user()->role === 'admin')
                            @if(request('seller_id'))
                                Mobil Seller:
                                {{ $cars->first() && $cars->first()->seller ? $cars->first()->seller->name : 'Tidak Diketahui' }}
                            @else
                                Kelola Semua Mobil
                            @endif
                        @else
    ...
    {{-- Filter Area/Kota (admin & seller) --}}
    <div class="mb-4">
        <form action="{{ route('cars.index') }}" method="GET" id="filterLocationForm" class="d-flex flex-wrap align-items-center gap-2">
            @if(Auth::user()->role === 'admin' && request('seller_id'))
                <input type="hidden" name="seller_id" value="{{ request('seller_id') }}">
            @endif
            <label for="filterLocation" class="form-label mb-0 fw-semibold" style="font-size: 14px;">
                <i class="fas fa-map-marker-alt me-1 text-danger"></i>Area/Kota:
            </label>
            <select name="location" id="filterLocation" class="form-select form-select-sm" style="max-width: 220px;" onchange="document.getElementById('filterLocationForm').submit();">
            ...
            @if(request('location'))
                <a href="{{ route('cars.index', Auth::user()->role === 'admin' && request('seller_id') ? ['seller_id' => request('seller_id')] : []) }}" class="btn btn-sm btn-outline-secondary">Reset</a>
            @endif
        </form>
    </div>
                    ...
                    <div class="car-card-body">
                        <div class="car-brand-section">
                            ...
                            @if(Auth::user()->role === 'seller' && $car->status == 'rejected')
                                @php
                                    $lastApprovalRejection = $car->approvals->where('action', 'rejected')->sortByDesc(function ($a) {
                                        return $a->approved_at ?? $a->created_at;
                                    })->first();
                                @endphp
                                @if($lastApprovalRejection)
                                    <div
                                        style="margin-top: 10px; padding: 10px 12px; background: #fee2e2; border-left: 3px solid #ef4444; border-radius: 5px;">
                                        <p
                                            style="margin: 0; font-size: 11px; color: #7f1d1d; display: flex; align-items: center; gap: 6px;">
                                ...
                                @if($unpublishReport)
                                    <div
                                        style="margin-top: 10px; padding: 8px 12px; background: #fef3c7; border-left: 3px solid #f59e0b; border-radius: 5px;">
                                        <p
                                            style="margin: 0; font-size: 11px; color: #92400e; display: flex; align-items: center; gap: 6px;">
                                        ...
                                        <p style="margin: 4px 0 0 0; font-size: 10px; color: #78350f;">
                                            <a href="{{ route('seller.reports.show', $unpublishReport) }}"
                        ...
                        <div class="car-details-list">
                            ...
                            <div class="car-detail-row">
                                ...
                                <div class="detail-text">
                                    <span class="detail-label">Bahan bakar:</span>
                                    <span class="detail-value">
                                        @if($car->bahan_bakar)
                                            <i class="fas fa-gas-pump me-1" style="font-size: 0.85em; opacity: 0.9;"></i>{{ $car->bahan_bakar }}
                                        @else
                                            -
                                        @endif
                                    </span>
                        ...
                        <div class="car-card-actions">
                            ...
                            <form id="delete-car-form-{{ $car->id }}" action="{{ route('cars.destroy', $car->id) }}" method="POST"
                                style="display: none;">
                                @csrf
                                @method('DELETE')
                            </form>
    ...
    @push('scripts')
        ...
        <script>
            function confirmDeleteCar(button) {
                ...
                let imageHtml = '';
                if (carImage) {
                    imageHtml = `<div style="width: 60px; height: 60px; border-radius: 8px; overflow: hidden; flex-shrink: 0; border: 1px solid #e9ecef;">
                            <img src="${carImage}" style="width: 100%; height: 100%; object-fit: cover;">
                        </div>`;
                } else {
                    imageHtml = `<div style="width: 40px; height: 40px; border-radius: 5px; background: linear-gradient(135deg, #dc2626, #991b1b); display: flex; align-items: center; justify-content: center; color: #fff; font-size: 16px; flex-shrink: 0;">
                            <i class="fas fa-car"></i>
                        </div>`;
                }
