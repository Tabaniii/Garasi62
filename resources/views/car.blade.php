...
@section('content')
                        ...
                        <div class="car__filter">
                            ...
                            <form action="{{ route('cars') }}" method="GET" id="filterForm">
                                ...
                                <select name="brand" onchange="document.getElementById('filterForm').submit();">
                                    <option value="" data-display="Brand">Pilih Brand</option>
                                ...
                                <select name="tahun" onchange="document.getElementById('filterForm').submit();">
                                    <option value="" data-display="Tahun">Pilih Tahun</option>
                                ...
                                <select name="tipe" onchange="document.getElementById('filterForm').submit();">
                                    <option value="" data-display="Tipe">Pilih Tipe</option>
                                    <option value="rent" {{ request('tipe') == 'rent' ? 'selected' : '' }}>Disewakan</option>
                                    <option value="buy" {{ request('tipe') == 'buy' ? 'selected' : '' }}>Dijual</option>
                                </select>

                                <select name="transmisi" onchange="document.getElementById('filterForm').submit();">
                                    <option value="" data-display="Transmisi">Pilih Transmisi</option>
                                ...
                                <select name="metode" onchange="document.getElementById('filterForm').submit();">
                                    <option value="" data-display="Metode">Pilih Metode</option>
                                ...
                                <select name="kapasitasmesin" onchange="document.getElementById('filterForm').submit();">
                                    <option value="" data-display="Kapasitas Mesin">Pilih Kapasitas Mesin</option>
                                ...
                                <select name="bahan_bakar" onchange="document.getElementById('filterForm').submit();">
                                    <option value="" data-display="Bahan Bakar">Pilih Bahan Bakar</option>
                                    <option value="Bensin" {{ request('bahan_bakar') == 'Bensin' ? 'selected' : '' }}>Bensin
                                    </option>
                                    <option value="Solar" {{ request('bahan_bakar') == 'Solar' ? 'selected' : '' }}>Solar
                                    </option>
                                    <option value="Listrik" {{ request('bahan_bakar') == 'Listrik' ? 'selected' : '' }}>
                                        Listrik</option>
                                    @foreach($bahanBakarList as $bahan)
                                        @if(!in_array($bahan, ['Bensin', 'Solar', 'Listrik']))
                                            <option value="{{ $bahan }}" {{ request('bahan_bakar') == $bahan ? 'selected' : '' }}>
                                                {{ $bahan }}</option>
                                        @endif
                                    @endforeach
                                </select>
                                ...
                                <select name="location" onchange="document.getElementById('filterForm').submit();">
                                    <option value="" data-display="Area/Kota">Pilih Area/Kota</option>
                                ...
                                <div class="filter-price">
                                    ...
                                    <div class="price-range-wrap">
                                        <div class="price-display"
                                            style="margin-bottom: 15px; padding: 12px; background: #f8f9fa; border-radius: 5px; text-align: center;">
                                            <div id="filterAmount"
                                                style="font-size: 14px; font-weight: 700; color: #dc2626;">
                                                Rp {{ number_format($minPrice ?? 0, 0, ',', '.') }} - Rp
                                                {{ number_format($maxPrice ?? 1000000000, 0, ',', '.') }}
                                            </div>
                                        </div>
                                        ...
                                        <div class="range-slider">
                                            ...
                                            <input type="hidden" name="max_price" id="maxPrice"
                                                value="{{ request('max_price', $maxPrice ?? 1000000000) }}">
                                        </div>
                                    </div>
                                </div>
                        ...
                        <div class="row">
                            <div class="col-lg-6 col-md-6">
                                <div class="car__filter__option__item">
                                    <h6>Tampilkan Per Halaman</h6>
                                    <form action="{{ route('cars') }}" method="GET" id="perPageForm"
                                        style="display: inline;">
                                        @foreach(request()->except('per_page', 'page') as $key => $value)
                                            <input type="hidden" name="{{ $key }}" value="{{ $value }}">
                                        @endforeach
                                        <select name="per_page" onchange="document.getElementById('perPageForm').submit();">
                                            <option value="9" {{ request('per_page', 9) == 9 ? 'selected' : '' }}>9 Mobil
                                            </option>
                                            <option value="15" {{ request('per_page') == 15 ? 'selected' : '' }}>15 Mobil
                                            </option>
                                            <option value="20" {{ request('per_page') == 20 ? 'selected' : '' }}>20 Mobil
                                            </option>
                                        </select>
                                    </form>
                                </div>
                                ...
                                <div class="car__filter__option__item car__filter__option__item--right">
                                    ...
                                    <form action="{{ route('cars') }}" method="GET" id="sortForm" style="display: inline;">
                                        ...
                                        <select name="sort_option" onchange="handleSortChange(this.value);">
                                            ...
                                            <option value="harga_desc" {{ request('sort_by') == 'harga' && request('sort_order') == 'desc' ? 'selected' : '' }}>Harga: Tertinggi</option>
                                            <option value="harga_asc" {{ request('sort_by') == 'harga' && request('sort_order') == 'asc' ? 'selected' : '' }}>Harga: Terendah</option>
                                            <option value="tahun_desc" {{ request('sort_by') == 'tahun' && request('sort_order') == 'desc' ? 'selected' : '' }}>Tahun: Terbaru</option>
                                            <option value="tahun_asc" {{ request('sort_by') == 'tahun' && request('sort_order') == 'asc' ? 'selected' : '' }}>Tahun: Terlama</option>
                                        </select>
                                        <input type="hidden" name="sort_by" id="sortBy"
                                            value="{{ request('sort_by', 'created_at') }}">
                                        <input type="hidden" name="sort_order" id="sortOrder"
                                ...
                                <div class="car-card-modern animate-fade-in-up" data-animation-delay="{{ $index * 0.1 }}">
                                    ...
                                    <div class="car-card-image-wrapper">
                                        ...
                                        <div class="car-card-overlay">
                                            <span
                                                class="car-type-badge {{ $car->tipe == 'buy' ? 'badge-sale' : 'badge-rent' }}">
                                                {{ $car->tipe == 'rent' ? 'Disewakan' : 'Dijual' }}
                                            </span>
                                            @if($car->stock)
                                                <span
                                                    class="stock-badge {{ strtolower($car->stock) === 'tersedia' ? 'stock-available' : 'stock-limited' }}">
                                                    {{ $car->stock }}
                                                </span>
                                            @endif
                                        </div>
                                    ...
                                    <div class="car-card-content">
                                        ...
                                        <div class="car-card-specs">
                                            <div class="spec-item-modern">
                                                ...
                                                <div class="spec-content">
                                                    ...
                                                    <span
                                                        class="spec-value">{{ number_format($car->kilometer ?? 0, 0, ',', '.') }}
                                                        km</span>
                                                </div>
                                            </div>
                                            <div class="spec-item-modern">
                                            ...
                                            <div class="spec-item-modern">
                                                <div class="spec-icon">
                                                    <i class="fas fa-gas-pump" title="Bahan bakar"></i>
                                                </div>
                                                <div class="spec-content">
                                                    <span class="spec-label">Bahan bakar</span>
                                                    <span class="spec-value">
                                                        @if($car->bahan_bakar)
                                                            <i class="fas fa-gas-pump" style="margin-right: 4px; font-size: 0.9em; opacity: 0.9;" title="Bahan bakar"></i>{{ $car->bahan_bakar }}
                                                        @else
                                                            -
                                                        @endif
                                                    </span>
                                        ...
                                        <div class="car-card-actions">
                                            <a href="{{ route('car.details', $car->uuid) }}" class="btn-detail">
                                                <i class="fa fa-eye"></i>
                                            ...
                                            @auth
                                                @if(auth()->user()->role === 'buyer')
                                                    ...
                                                    @endphp
                                                    @if($seller)
                                                        <a href="{{ route('chat.seller', $seller->id) }}?car_id={{ $car->id }}"
                                                            class="btn-chat">
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            // Step nominal per 50 juta (slider hanya bergerak kelipatan 50 juta)
            var STEP_50JUTA = 50000000;

            var dataMin = {{ $minPrice ?? 0 }};
            var dataMax = {{ $maxPrice ?? 1000000000 }};
            // Rentang slider: 0 sampai kelipatan 50 juta di atas harga max data
            var sliderMin = 0;
            var sliderMax = Math.ceil(dataMax / STEP_50JUTA) * STEP_50JUTA;
            if (sliderMax < STEP_50JUTA) sliderMax = STEP_50JUTA;

            var currentMin = {{ request('min_price', $minPrice ?? 0) }};
            var currentMax = {{ request('max_price', $maxPrice ?? 1000000000) }};
            // Snap ke kelipatan 50 juta
            currentMin = Math.floor(currentMin / STEP_50JUTA) * STEP_50JUTA;
            currentMax = Math.ceil(currentMax / STEP_50JUTA) * STEP_50JUTA;
            currentMin = Math.max(sliderMin, currentMin);
            currentMax = Math.min(sliderMax, currentMax);
            if (currentMin > currentMax) {
                currentMin = Math.floor(currentMax / STEP_50JUTA) * STEP_50JUTA;
                if (currentMin > currentMax) currentMin = Math.max(sliderMin, currentMax - STEP_50JUTA);
            }
